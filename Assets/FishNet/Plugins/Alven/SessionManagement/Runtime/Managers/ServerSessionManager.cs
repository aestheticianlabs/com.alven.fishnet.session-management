...
namespace FishNet.Alven.SessionManagement
{
	...
	public sealed class ServerSessionManager : MonoBehaviour
	{
		...
		/// <summary>
		/// Returns whether a player with the provided authentication PlayerId is in the current session.
		/// </summary>
		...
		internal bool SetupPlayerConnection(
			SessionAuthenticator authenticator, string playerId, NetworkConnection connection
		)
		{
		...
		/// </summary>
		private void BroadcastPlayerConnectionChange(
			SessionPlayer player, PlayerConnectionState state, bool requireAuthenticated = true
		)
		{
